...
```{r message=FALSE, warning=FALSE}
getwd
temp_fish <- readr::read_csv("Gaeta_etal_CLC_data.csv")
```
...
```{r}
select(fish, lakeid, scalelength) ## I akse why we didn't use this because it seems more efficient than pipe, but then i was explained pipe allows us to attach additional requests in one code; something that select can't do 
```
...
fish %>% 
  select(lakeid, length, fish_id, scalelength) %>% 
  filter(lakeid=="AL") %>% ## we only want lakes in alabama
  arrange(fish_id) %>% 
  mutate(scale_ratio=(length/scalelength)) ## here we're adding a new column of ratio. we called the new column "scaleratio". then we tell mutate what to do 
```
...
fish %>%
  ...
  arrange(lakeid) %>%
  filter(lakeid== "AL") %>% ## filter will select a specfic column and then specify what we want to 
  mutate(radii_ratio=(radii_length_mm/scalelength))
